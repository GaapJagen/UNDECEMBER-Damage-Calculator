...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Label(root, text='Tooltip is built first, followed by additional damage, Critical, Double, and Triple.', foreground='#555').pack(anchor='w', pady=(0, 8))
        ...
        notebook.add(constellation_tab, text='Constellations')
        notebook.add(editor_tab, text='Node Editor')
        ...
        ttk.Label(right, text='RESULT / BREAKDOWN', font=('Segoe UI', 12, 'bold')).pack(anchor='w')
        self.output = tk.Text(right, wrap='word', font=('Consolas', 10), state='disabled')
        self.output.pack(fill='both', expand=True, pady=(8, 8))
        buttons = ttk.Frame(right)
        buttons.pack(fill='x')
        ttk.Button(buttons, text='CALCULATE', command=self.calculate).pack(side='left', padx=(0, 6))
    ...
    def _editor_choose_asset(self, key):
        ...
        path=filedialog.askopenfilename(title=f'Choose image: {key}',
        ...
        try:
            ...
            if Image is not None:
                ...
                if Path(path).suffix.lower() != '.png':
                    raise ValueError('PNG image is required when Pillow is unavailable.')
            ...
            messagebox.showerror('Image', f'Failed to set image:\n{e}')

    def _editor_reset_assets(self):
        from tkinter import messagebox
        if not messagebox.askyesno('Reset Images','Delete custom editor images?'):
            return
        for key in ('background','node','branch'):
            path=ASSET_DIR / f'{key}.png'
            try:
    ...
    def _editor_refresh_asset_status(self):
        active=[{'background':'background','node':'nodes','branch':'branches'}[k] for k,v in self.editor_assets.items() if v]
        text='Custom: '+', '.join(active)+'.' if active else 'Using default images.'
    ...
    def _constellation_runtime(self, p):
        ...
        ttk.Label(top,text='Active Constellations',font=('Segoe UI',14,'bold')).pack(side='left')
        ttk.Button(top,text='Load JSON',command=self._runtime_load_json).pack(side='right')
        ttk.Button(top,text='Reset Nodes',command=self._runtime_reset).pack(side='right',padx=4)
        self.runtime_status=ttk.Label(outer,text='Load a constellation JSON file.'); self.runtime_status.pack(anchor='w',pady=(4,6))
        ...
        ttk.Label(right,text='ACTIVE BONUSES',font=('Segoe UI',11,'bold')).pack(anchor='w')
        self.runtime_bonus=ttk.Label(right,text='No active nodes.',justify='left',wraplength=300)
        self.runtime_bonus.pack(anchor='w',pady=(8,12))
        ttk.Label(right,text='Access Rule',font=('Segoe UI',10,'bold')).pack(anchor='w')
        ttk.Label(right,text='A root node can be taken immediately.\nOther nodes are available if at least one linked node is already active.',justify='left',wraplength=300).pack(anchor='w',pady=(4,0))
    ...
    def _runtime_load_json(self):
        from tkinter import filedialog,messagebox
        path=filedialog.askopenfilename(title='Load Constellations',filetypes=[('JSON','*.json'),('All files','*.*')])
        if not path:return
        try:
            with open(path,'r',encoding='utf-8') as fh: data=json.load(fh)
            consts=data.get('constellations',data)
            if not isinstance(consts,dict): raise ValueError('JSON does not contain a constellations object.')
            normalized={}
            for ck,branches in consts.items():
                ...
                for bk,nodes in branches.items():
                    ...
                    for n in nodes or []:
                        ...
                        q=dict(n); q.setdefault('x',.5); q.setdefault('y',.5); q.setdefault('name','Node'); q.setdefault('effect','No parameter'); q.setdefault('value',0); q.setdefault('parent_id',None); q.setdefault('links',[]); q.setdefault('root', q.get('parent_id') is None)
                        out.append(q)
                    normalized[str(ck)][str(bk)]=out
            self.runtime_nodes=normalized
            self.runtime_active=set(); self.runtime_const.set(1)
            self._runtime_rebuild_branches(); self._runtime_draw(); self._runtime_update_bonuses()
            self.runtime_status.config(text=f'Constellations loaded: {len(normalized)}.')
        except Exception as e: messagebox.showerror('Constellations',f'Failed to load JSON:\n{e}')
    ...
    def _runtime_rebuild_branches(self):
        ...
        for k in keys:
            ttk.Radiobutton(self.runtime_branch_bar,text=f'Branch {k}',value=k,variable=self.runtime_branch,command=self._runtime_branch_changed).pack(side='left',padx=3)
    ...
    def _runtime_click(self,event):
        ...
        if nid in self.runtime_active:
            ...
            msg=f"Disabled: {hit.get('name','Node')}"
        elif self._runtime_available(hit,allmap):
            self.runtime_active.add(nid); msg=f"Activated: {hit.get('name','Node')}"
        else:
            msg='Node unavailable: activate a linked prerequisite node first.'
    ...
    def _runtime_draw(self):
        ...
        for n in nodes:
            ...
            c.create_text(x+20,y-14,text=n.get('name','Node'),anchor='w',fill='#e6f3f5' if available else '#71858a',font=('Segoe UI',9,'bold' if active else 'normal'))
        c.create_text(10,10,text=f'Constellation {self.runtime_const.get()} — branch {self.runtime_branch.get()}',anchor='nw',fill='#d7e7ea',font=('Segoe UI',11,'bold'))
    ...
    def _runtime_update_bonuses(self):
        ...
        self.runtime_bonus.config(text='\n'.join(f'{k}: +{v:g}%' for k,v in totals.items()) if totals else 'No active nodes.')
    ...
    def _constellation_editor(self, p):
        ...
        ttk.Label(top, text='Node Layout Editor', font=('Segoe UI', 14, 'bold')).pack(side='left')
        ttk.Label(top, text='Drag nodes with the mouse. When finished, click “Export JSON”.', foreground='#666').pack(side='left', padx=14)

        controls = ttk.Frame(outer)
        controls.pack(fill='x', pady=(0, 6))
        ttk.Label(controls, text='Constellation:').pack(side='left')
        ...
        ttk.Label(controls, text='   Branches:').pack(side='left', padx=(10, 0))
        ...
        ttk.Button(controls, text='+ Branch', command=self._editor_add_branch).pack(side='left', padx=(8,3))
        ttk.Button(controls, text='− Branch', command=self._editor_delete_branch).pack(side='left', padx=3)
        ttk.Button(controls, text='+ Node', command=self._editor_add_node).pack(side='left', padx=(10,3))
        ttk.Button(controls, text='− Node', command=self._editor_delete_node).pack(side='left', padx=3)
        ttk.Button(controls, text='↔ Link', command=self._editor_start_link).pack(side='left', padx=(10,3))
        ttk.Button(controls, text='− Unlink', command=self._editor_unlink_selected).pack(side='left', padx=3)
        ttk.Button(controls, text='Save JSON', command=self._editor_save_json).pack(side='right', padx=3)
        ttk.Button(controls, text='Load JSON', command=self._editor_load_json).pack(side='right', padx=3)
        ttk.Button(controls, text='Export JSON', command=self._editor_export_json).pack(side='right', padx=3)

        assets=ttk.LabelFrame(outer,text='Editor Images',padding=6)
        assets.pack(fill='x', pady=(0,6))
        ttk.Button(assets,text='Background…',command=lambda:self._editor_choose_asset('background')).pack(side='left',padx=2)
        ttk.Button(assets,text='Node Icon…',command=lambda:self._editor_choose_asset('node')).pack(side='left',padx=2)
        ttk.Button(assets,text='Branch Icon…',command=lambda:self._editor_choose_asset('branch')).pack(side='left',padx=2)
        ttk.Button(assets,text='Reset Images',command=self._editor_reset_assets).pack(side='left',padx=(10,2))
        self.editor_asset_status=ttk.Label(assets,text='Using default images.',foreground='#666')
        ...
        lf=ttk.LabelFrame(side,text='Selected Node',padding=8)
        lf.pack(fill='x',pady=(0,8))
        self.editor_node_label=ttk.Label(lf,text='No node selected',font=('Segoe UI',10,'bold'),wraplength=230)
        ...
        ttk.Label(lf,text='Node Name (automatic):').pack(anchor='w',pady=(8,2))
        ...
        ttk.Label(lf,text='Node Parameter:').pack(anchor='w',pady=(8,2))
        ...
        ttk.Label(lf,text='Value %:').pack(anchor='w',pady=(8,2))
        ...
        ttk.Label(lf,text='Node Description (automatic):').pack(fill='x',pady=(8,2))
        ...
        ttk.Checkbutton(lf,text='Root Node (starting node)',variable=self.editor_root_var).pack(anchor='w',pady=(8,0))
        ttk.Label(lf,text='A root node is immediately available when entering a branch. A branch may have multiple root nodes.',foreground='#666',wraplength=230,justify='left').pack(anchor='w',pady=(2,0))

        ttk.Button(lf,text='Apply to Node',command=self._editor_apply_node_fields).pack(fill='x',pady=(8,0))

        self.editor_node_hint=ttk.Label(lf,text='Left-click a node → choose its parameter and value on the right. These values are saved to JSON.',foreground='#666',wraplength=230,justify='left')
        self.editor_node_hint.pack(anchor='w',pady=(8,0))

        lf2=ttk.LabelFrame(side,text='Help',padding=8)
        lf2.pack(fill='x',pady=(0,8))
        ttk.Label(lf2,text=(
            'Positions are stored as relative coordinates from 0..1 within the map.\n\n'
            '• Constellation — I–IX\n'
            '• Each constellation may have any number of branches\n'
            '• Nodes can be moved independently\n'
            '• “Add Node” from a selected node creates a child node and link\n'
            '• Multiple nodes can be made roots — each becomes immediately available\n'
            '• The JSON can be sent to me — I can use it as the exact layout'
        ),justify='left',wraplength=235).pack(anchor='w')

        self.editor_status=ttk.Label(side,text='Ready.',foreground='#666',wraplength=235,justify='left')
  ...
  "1": {
    "1": [
      ...
      },
      {
        ...
        "name": "Attack Damage Amplification: +3%",
        ...
        "description": "Attack Damage Amplification: +3%"
      }
    ],
    "3": [
      {
      ...
      },
      {
        ...
        "name": "Spell Damage Amplification: +3%",
        ...
        "description": "Spell Damage Amplification: +3%"
      }
    ],
    "4": [
      {
    ...
    "3": [
      {
        ...
        "description": "Elemental Damage: +10%"
      },
      {
        "id": "2-3-2",
      ...
      },
      {
        ...
        "name": "Fire Damage: +3%(Amplification)",
        ...
        "description": "Fire Damage: +3%(Amplification)"
      },
      {
        "id": "2-3-6",
      ...
      },
      {
        ...
        "name": "Cold Damage: +3%(Amplification)",
        ...
        "description": "Cold Damage: +3%(Amplification)"
      },
      {
        "id": "2-3-10",
        ...
        "description": "Lightning Damage: +5%"
      },
      {
        "id": "2-3-11",
        "x": 0.3887376237623762,
        "y": 0.5593055555555554,
        "name": "Lightning Damage: +15%",
        "effect": "Lightning Damage Increase",
        "value": 15.0,
        "type": "node",
        "parent_id": "2-3-10",
        ...
        "description": "Lightning Damage: +15%"
      },
      {
        "id": "2-3-12",
      ...
      },
      {
        ...
        "name": "Lightning Damage: +3%(Amplification)",
        ...
        "description": "Lightning Damage: +3%(Amplification)"
      },
      {
        "id": "2-3-14",
        "x": 0.2974009900990099,
        "y": 0.6714351851851851,
        "name": "Poison Damage: +5%",
        ...
        "description": "Poison Damage: +5%"
      },
      {
        "id": "2-3-15",
        "x": 0.4388613861386138,
        "y": 0.6565277777777777,
        "name": "Poison Damage: +15%",
        "effect": "Poison Damage Increase",
        "value": 15.0,
        "type": "node",
        "parent_id": "2-3-14",
      ...
      },
      {
        ...
        "name": "Poison Damage: +3%(Amplification)",
        ...
        "description": "Poison Damage: +3%(Amplification)"
      }
    ],
    "4": [
      {
      ...
      },
      {
        ...
        "name": "Physical Damage: +3%(Amplification)",
        ...
        "description": "Physical Damage: +3%(Amplification)"
      },
      {
        "id": "2-4-6",
  ...
  "3": {
    ...
    "3": [
      ...
      },
      {
        ...
        "root": false,
        "description": "Elemental Damage: +5%"
      },
      {
        "id": "3-3-3",
        "x": 0.439480198019802,
        "y": 0.588927680798005,
        "name": "Elemental Damage: +20%",
        "effect": "Elemental Damage Increase",
        "value": 20.0,
        "type": "node",
        "parent_id": "3-3-2",
        ...
        "root": false,
        "description": "Elemental Damage: +20%"
      },
      {
        "id": "3-3-4",
      ...
      },
      {
        ...
        "root": false,
        "description": "Melee Damage: +5%"
      },
      {
        "id": "3-3-6",
        "x": 0.49950495049504956,
        "y": 0.47795511221945136,
        "name": "Melee Damage: +20%",
        "effect": "Melee Damage Increase",
        "value": 20.0,
        "type": "node",
        "parent_id": "3-3-5",
      ...
      },
      {
        ...
        "name": "Melee Damage: +8%(Amplification)",
        ...
        "root": false,
        "description": "Melee Damage: +8%(Amplification)"
      },
      {
        "id": "3-3-9",
      ...
      },
      {
        ...
        "root": false,
        "description": "Physical Damage: +20%"
      },
      {
        "id": "3-3-11",
    ...
    "4": [
      {
        ...
        "root": true,
        "description": "Projectile Damage: +10%"
      },
      {
        "id": "3-4-2",
        "x": 0.3964108910891089,
        "y": 0.44284289276807975,
        "name": "Projectile Damage: +5%",
        "effect": "Projectile Damage Increase",
        "value": 5.0,
        "type": "node",
        "parent_id": "3-4-1",
        ...
        "root": false,
        "description": "Projectile Damage: +5%"
      },
      {
        "id": "3-4-3",
        "x": 0.50259900990099,
        "y": 0.4081296758104737,
        "name": "Projectile Damage: +20%",
        "effect": "Projectile Damage Increase",
        "value": 20.0,
        "type": "node",
        "parent_id": "3-4-2",
      ...
      },
      {
        ...
        "name": "Projectile Damage: +8%(Amplification)",
        ...
        "root": false,
        "description": "Projectile Damage: +8%(Amplification)"
      },
      {
        "id": "3-4-6",
  ...
  "4": {
    ...
    "3": [
      ...
      },
      {
        ...
        "root": false,
        "description": "Area Damage: +5%"
      },
      {
        "id": "4-3-4",
        "x": 0.31225247524752475,
        "y": 0.42788029925187027,
        "name": "Melee Damage: +10%",
        "effect": "Melee Damage Increase",
        "value": 10.0,
        "type": "node",
        "parent_id": "4-3-1",
      ...
      },
      {
        ...
        "y": 0.464638403990025,
        "name": "Area Damage: +5%(Amplification)",
        ...
        "root": false,
        "description": "Area Damage: +5%(Amplification)"
      },
      {
        "id": "4-3-9",
    ...
    "5": [
      ...
      },
      {
        ...
        "name": "Elemental Damage: +5%(Amplification)",
        ...
        "root": false,
        "description": "Elemental Damage: +5%(Amplification)"
      },
      {
        "id": "4-5-10",
    ...
    "6": [
      ...
      },
      {
        ...
        "name": "Physical Damage: +5%(Amplification)",
        "effect": "Physical Damage Amplification",
        "value": 5.0,
        "type": "node",
        "parent_id": "4-6-8",
        ...
        "root": false,
        "description": "Physical Damage: +5%(Amplification)"
      },
      {
        "id": "4-6-10",
  ...
  "5": {
    "1": [
      ...
      },
      {
        ...
        "name": "Spell Damage Amplification: +5%",
        "effect": "Spell Damage Amplification",
        "value": 5.0,
        "type": "node",
        "parent_id": "5-1-4",
        ...
        "root": false,
        "description": "Spell Damage Amplification: +5%"
      },
      {
        "id": "5-1-6",
    ...
    "2": [
      ...
      },
      {
        ...
        "name": "Attack Damage Amplification: +5%",
        "effect": "Attack Damage Amplification",
        "value": 5.0,
        "type": "node",
        "parent_id": "5-2-4",
        ...
        "root": false,
        "description": "Attack Damage Amplification: +5%"
      },
      {
        "id": "5-2-6",
    ...
    "3": [
      ...
      },
      {
        ...
        "root": false,
        "description": "Damage: +30%(Amplification)"
      },
      {
        "id": "5-3-4",
        "x": 0.6007425742574257,
      ...
      },
      {
        ...
        "root": false,
        "description": "Critical Damage: +10%"
      },
      {
        "id": "5-3-7",
        "x": 0.49950495049504956,
  ...
  "6": {
    "1": [
      ...
      },
      {
        ...
        "name": "Area Damage: +6%(Amplification)",
        "effect": "Area Damage Amplification",
        "value": 6.0,
        "type": "node",
        "parent_id": "6-1-8",
        ...
        "root": false,
        "description": "Area Damage: +6%(Amplification)"
      },
      {
        "id": "6-1-10",
      ...
      },
      {
        ...
        "name": "Strike Damage: +6%(Amplification)",
        ...
        "root": false,
        "description": "Strike Damage: +6%(Amplification)"
      },
      {
        "id": "6-1-14",
    ...
    "2": [
      ...
      },
      {
        ...
        "root": false,
        "description": "Damage: +5%(Amplification)"
      },
      {
        "id": "6-2-8",
      ...
      },
      {
        ...
        "root": false,
        "description": "Elemental Damage: +15%"
      },
      {
        "id": "6-2-15",
  ...
  "7": {
    "1": [
      ...
      },
      {
        ...
        "root": false,
        "description": "Strike Damage: +5%"
      },
      {
        "id": "7-1-11",
        "x": 0.5939851485148514,
        "y": 0.44640897755610964,
        "name": "Strike Damage: +15%",
        "effect": "Strike Damage Increase",
        "value": 15.0,
        "type": "node",
        "parent_id": "7-1-10",
      ...
      },
      {
        ...
        "name": "Strike Damage: -8%(Amplification)",
        "effect": "Strike Damage Amplification",
        "value": -8.0,
        "type": "node",
        "parent_id": "7-1-16",
        ...
        "root": false,
        "description": "Strike Damage: -8%(Amplification)"
      }
    ],
    "2": [
      {
      ...
      },
      {
        ...
        "root": false,
        "description": "Critical Damage: +15%"
      },
      {
        "id": "7-2-4",
      ...
      },
      {
        ...
        "root": false,
        "description": "Damage: +3%(Amplification)"
      }
    ],
    "3": [
      {
    ...
    "4": [
      ...
      },
      {
        ...
        "name": "Melee Damage: +5%(Amplification)",
        "effect": "Melee Damage Amplification",
        "value": 5.0,
        "type": "node",
        "parent_id": "7-4-4",
        ...
        "root": false,
        "description": "Melee Damage: +5%(Amplification)"
      },
      {
        "id": "7-4-6",
      ...
      },
      {
        ...
        "root": false,
        "description": "Projectile Damage: +15%"
      },
      {
        "id": "7-4-16",
      ...
      },
      {
        ...
        "root": false,
        "description": "Projectile Damage: +30%"
      },
      {
        "id": "7-4-18",
        "x": 0.6737623762376236,
      ...
      },
      {
        ...
        "name": "Projectile Damage: +5%(Amplification)",
        "effect": "Projectile Damage Amplification",
        "value": 5.0,
        "type": "node",
        "parent_id": "7-4-18",
        ...
        "root": false,
        "description": "Projectile Damage: +5%(Amplification)"
      }
    ],
    "5": [
      {
      ...
      },
      {
        ...
        "root": false,
        "description": "Physical Damage: +30%"
      },
      {
        "id": "7-5-9",
        "x": 0.532549504950495,
      ...
      },
      {
        ...
        "root": true,
        "description": "Lightning Damage: +10%"
      },
      {
        "id": "7-5-16",
      ...
      },
      {
        ...
        "root": false,
        "description": "Lightning Damage: +30%"
      }
    ],
    "6": [
      {
        ...
        "links": [],
        "root": true,
        "description": "Damage: +2%(Amplification)"
      },
      {
        "id": "7-6-2",
  ...
  "9": {
    ...
    def _node_description(self, effect, value):
        ...
        try:
            ...
            v = 0.0
        labels = {
            'Attack Damage Increase': 'Attack Damage',
            'Attack Damage Amplification': 'Attack Damage Amplification',
            'Spell Damage Increase': 'Spell Damage',
            'Spell Damage Amplification': 'Spell Damage Amplification',
            'Physical Damage Increase': 'Physical Damage',
            'Physical Damage Amplification': 'Physical Damage (Amplification)',
            'Fire Damage Increase': 'Fire Damage',
            'Fire Damage Amplification': 'Fire Damage (Amplification)',
            'Cold Damage Increase': 'Cold Damage',
            'Cold Damage Amplification': 'Cold Damage (Amplification)',
            'Lightning Damage Increase': 'Lightning Damage',
            'Lightning Damage Amplification': 'Lightning Damage (Amplification)',
            'Poison Damage Increase': 'Poison Damage',
            'Poison Damage Amplification': 'Poison Damage (Amplification)',
            'Elemental Damage Increase': 'Elemental Damage',
            'Elemental Damage Amplification': 'Elemental Damage (Amplification)',
            'Projectile Damage Increase': 'Projectile Damage',
            'Projectile Damage Amplification': 'Projectile Damage (Amplification)',
            'Melee Damage Increase': 'Melee Damage',
            'Melee Damage Amplification': 'Melee Damage (Amplification)',
            'Area Damage Increase': 'Area Damage',
            'Area Damage Amplification': 'Area Damage (Amplification)',
            'Strike Damage Increase': 'Strike Damage',
            'Strike Damage Amplification': 'Strike Damage (Amplification)',
            'Double Maximum Damage Increase': 'Maximum Damage (Double)',
            'Triple Maximum Damage Increase': 'Maximum Damage (Triple)',
            'Maximum Damage Increase': 'Maximum Damage',
            'Critical Damage Increase': 'Critical Damage',
            'Generic Damage Increase': 'Damage',
            'Generic Damage Amplification': 'Damage (Amplification)',
            'No parameter': '',
        }
        ...
        if effect == 'Triple Maximum Damage Increase':
            return f'{label}: +{vs}%'
        suffix = '(Amplification)' if effect.endswith('Amplification') and '(Amplification)' not in label else ''
        return f'{label}: +{vs}%'
    ...
    def _editor_apply_node_fields(self):
        ...
        if node is None:
            self.editor_status.config(text='Select a node first.')
            return
        try:
            value=float(self.editor_effect_value_var.get().replace(',','.').strip() or 0)
        except ValueError:
            self.editor_status.config(text='Value % must be a number.')
            return
        ...
        if make_root:
            node['root']=True
            node['parent_id']=None
            status=f"Saved: {node['effect']} = {value:g}%. Node set as root."
        else:
            node['root']=False
            if node.get('parent_id') is None:
                ...
                if linked:
                    ...
                    status=f"Saved: {node['effect']} = {value:g}%. Root status removed, parent: {linked[0].get('name','Node')}."
                else:
                    ...
                    status='Cannot remove root status: the node has no parent or link from which a parent can be restored.'
                
            else:
                status=f"Saved: {node['effect']} = {value:g}%."
    ...
    def _editor_selection_changed(self):
        ...
        self.editor_selected_id=None
        self.editor_drag_id=None
        self.editor_node_label.config(text='No node selected')
        ...
        self.editor_status.config(text=f'Constellation {chr(0x2160+c-1)}, branch {b}.')
        self._editor_draw()
    ...
    def _editor_add_branch(self, silent=False):
        ...
        branches[str(new_num)].append({
            'id':node_id,'x':0.5,'y':0.5,'name':'Node 1','effect':'No parameter','value':0,'type':'node','parent_id':None,'links':[],'root':True
        })
        self.editor_branch.set(new_num)
        self.editor_selected_id=node_id
        self._editor_rebuild_branch_buttons()
        self.editor_node_label.config(text='Node 1')
        self.editor_node_coords.config(text='X: 0.5000   Y: 0.5000')
        self._editor_fill_node_fields(branches[str(new_num)][0])
        if not silent:
            self.editor_status.config(text=f'Created branch {new_num} with one node.')
        self._editor_draw()
    ...
    def _editor_delete_branch(self):
        ...
        branches=self.editor_nodes[ckey]
        if len(branches)<=1:
            messagebox.showinfo('Delete Branch', 'Cannot delete the last branch of a constellation.\nAdd another branch before deleting this one.')
            return
        bkey=str(self.editor_branch.get())
        if bkey not in branches:
            return
        if not messagebox.askyesno('Delete branch', f'Delete branch {bkey} together with all its nodes?'):
            return
        del branches[bkey]
        ...
        self.editor_node_label.config(text='No node selected')
        self.editor_node_coords.config(text='X: —   Y: —')
        self.editor_status.config(text=f'Branch {bkey} deleted.')
        self._editor_draw()
    ...
    def _editor_draw(self):
        ...
        c.create_text(10,10,text=f'Constellation {chr(0x2160+self.editor_constellation.get()-1)} — branch {self.editor_branch.get()}',
    ...
    def _editor_mouse_down(self,event):
            ...
            if source_id and source_id != target_id:
                ...
                if src and dst:
                    ...
                    if target_id not in src['links']:
                        ...
                        self.editor_status.config(text=f'Link created: {src.get("name","Node")} ↔ {dst.get("name","Node")}.')
                    else:
                        self.editor_status.config(text='These nodes are already linked.')
            self.editor_link_mode=False
            self.editor_link_source_id=None
            self.editor_selected_id=node['id']
            self._editor_fill_node_fields(node)
            self.editor_node_label.config(text=node.get('name','Node'))
        ...
        self.editor_node_coords.config(text=f"X: {node['x']:.4f}   Y: {node['y']:.4f}")
        self._editor_fill_node_fields(node)
        self.editor_status.config(text='Node selected. Drag it with the mouse or edit its parameters on the right.')
        self._editor_draw()
    ...
    def _editor_add_node(self):
        ...
        if parent:
            ...
            status=f'Added {"Node "+str(n)} from {parent.get("name","selected node")}. Drag it to the desired position.'
        else:
            x,y=0.5,0.5
            parent_id=None
            status=f'Added Node {n} in the center. To create a child node, select a node first.'

        node={'id':f'{self.editor_constellation.get()}-{self.editor_branch.get()}-{n}',
              'x':x,'y':y,'name':f'Node {n}','effect':'No parameter','value':0,'type':'node','parent_id':parent_id,'links':[],'root':parent_id is None}
        nodes.append(node)
        self.editor_selected_id=node['id']
        self.editor_status.config(text=status)
        self.editor_node_label.config(text=node['name'])
    ...
    def _editor_start_link(self):
        node=self._editor_selected_node()
        if node is None:
            self.editor_status.config(text='Select the first node, click “↔ Link”, then click the second node.')
            return
        self.editor_link_mode=True
        self.editor_link_source_id=node['id']
        self.editor_status.config(text=f'Link mode: selected «{node.get("name","Node")}». Now click the second node.')
        self._editor_draw()
    ...
    def _editor_unlink_selected(self):
        ...
        if not links and not parent:
            self.editor_status.config(text='The selected node has no additional links.')
            return
        nodes=self._editor_current_nodes()
        by_id={n.get('id'):n for n in nodes}
        # Remove explicit links from both sides. parent_id is intentionally kept;
        # use «− Node» or JSON editing if the original tree parent must change.
        for lid in links:
            other=by_id.get(lid)
            if other and isinstance(other.get('links'),list):
                other['links']=[x for x in other['links'] if x!=node.get('id')]
        node['links']=[]
        self.editor_status.config(text='Additional links of the selected node removed.')
        self._editor_draw()
    ...
    def _editor_delete_node(self):
        ...
        if len(nodes)!=before:
            self.editor_status.config(text='Node deleted.')
        self.editor_selected_id=None
        self.editor_node_label.config(text='No node selected')
        self.editor_node_coords.config(text='X: —   Y: —')
        self._editor_fill_node_fields(None)
    ...
    def _editor_save_json(self):
        ...
        path=filedialog.asksaveasfilename(title='Save layout JSON',defaultextension='.json',
                                         filetypes=[('JSON','*.json'),('All files','*.*')],
                                         initialfile='constellation_layout.json')
        if not path: return
        Path(path).write_text(self._editor_json_text(),encoding='utf-8')
        self.editor_status.config(text=f'Saved:\n{path}')

    def _editor_export_json(self):
        from tkinter import filedialog
        path=filedialog.asksaveasfilename(title='Export for ChatGPT',defaultextension='.json',
                                         filetypes=[('JSON','*.json'),('All files','*.*')],
        ...
        try:
            ...
            copied='JSON was also copied to the clipboard.'
        except tk.TclError:
            copied=''
        self.editor_status.config(text=f'Exported:\n{path}\n{copied}\nSend me this JSON file.')

    def _editor_load_json(self):
        from tkinter import filedialog, messagebox
        path=filedialog.askopenfilename(title='Load layout JSON',
                                       filetypes=[('JSON','*.json'),('All files','*.*')])
        if not path: return
        try:
            ...
            if not isinstance(consts,dict): raise ValueError('Missing section constellations.')
            # Minimal validation: positions must be finite numbers in 0..1.
            for ckey,branches in consts.items():
                int(ckey)
                if not isinstance(branches,dict): raise ValueError('Invalid branch.')
                for bkey,nodes in branches.items():
                    int(bkey)
                    if not isinstance(nodes,list): raise ValueError('Invalid node list.')
                    for node in nodes:
                        x=float(node['x']); y=float(node['y'])
                        if not (0<=x<=1 and 0<=y<=1): raise ValueError('Coordinates must be 0..1.')
                        node.setdefault('name', f"Node {node.get('id','')}")
            ...
            self.editor_selected_id=None
            self._editor_draw()
            self.editor_status.config(text=f'Loaded:\n{path}')
        except Exception as exc:
            messagebox.showerror('Layout JSON Error',str(exc))
    ...
    def _character(self, p):
        lf = self.section(p, '1. CHARACTER PARAMETERS')
        headers = ['Parameter', 'MIN', 'MAX', 'INCREASE %', 'AMPLIFICATION %']
        ...
        ttk.Label(lf, text='Base Attack/Spell flat is first transformed by its Increase and Amplification; only the calculated result goes into the tooltip.', foreground='#8a4b00').grid(row=12, column=0, columnspan=5, sticky='w', pady=(5,0))
        ttk.Label(lf, text='Source').grid(row=13, column=0, sticky='w')
        ...
        ttk.Label(lf, text='Damage %').grid(row=14, column=0, sticky='w')
        self.multi_entry(lf, 'genericInc', '0').grid(row=14, column=1)
        ttk.Label(lf, text='Damage % (Amplification)').grid(row=14, column=2, sticky='e')
        self.multi_entry(lf, 'genericAmp', '0').grid(row=14, column=3)
        ttk.Label(lf, text='Global Increase is added to every type; Global Amplification is a separate multiplicative Amp.', foreground='#555').grid(row=15, column=0, columnspan=5, sticky='w')

    def _skill(self, p):
        lf = self.section(p, '2. SKILL PARAMETERS')
        ttk.Label(lf, text='Type').grid(row=0, column=0)
        ttk.Label(lf, text='Skill Damage %').grid(row=0, column=1)
        ttk.Label(lf, text='flat').grid(row=0, column=2)
        ttk.Label(lf, text='Tooltip MIN').grid(row=0, column=3)
        ttk.Label(lf, text='Tooltip MAX').grid(row=0, column=4)
        ...
        ttk.Label(
            lf,
            text='Tooltip is a separate input for the actual value displayed by the game. It is used for comparison and does not replace Skill Damage % or flat.',
            foreground='#555'
        ).grid(row=7, column=0, columnspan=5, sticky='w', pady=(6,0))
        ttk.Label(
            lf,
            text='If Tooltip MIN/MAX are filled in, they replace the calculated component at the main tooltip stage. All later stages (tags, additional damage, Double/Triple/Critical) operate on the entered tooltip.',
            foreground='#8a4b00'
        ).grid(row=8, column=0, columnspan=5, sticky='w')
    def _tags(self, p):
        lf = self.section(p, '3. TAG MODIFIERS')
        ...
        heads=['Tag','Increase %','Amplification %']
    ...
    def _special(self,p):
        lf=self.section(p,'4. ADDITIONAL DAMAGE (up to 5)')
        ...
        ttk.Label(lf,text='If Source = Target, the packet is included in that damage type’s main tooltip. Otherwise it is shown separately.').grid(row=6,column=0,columnspan=4,sticky='w')

    def _post(self,p):
        lf=self.section(p,'5. AFTER TOOLTIP')
    ...
    def calculate(self):
        ...
        comps={}; lines=[f'=== {APP_TITLE} ===','',f'Source: {source} | Calculated Base Damage: {fmt(bmin,bmax)}',f'Attack calculated: {fmt(atk_min,atk_max)} | factor {atk_factor:.8f}',f'Spell calculated: {fmt(spell_min,spell_max)} | factor {spell_factor:.8f}','', '1. MAIN TOOLTIP']
        ...
        for name,a,b,inc,amp,skill,skillflat,kmn,kmx in data:
            ...
            if has_entered_tooltip:
                ...
                lines.append(f'{name:10} {fmt(mn,mx)} | ENTERED TOOLTIP (calculation was {fmt(calc_mn,calc_mx)})')
            else:
        ...
        for i in range(1,6):
            ...
            if src not in comps or pct==0:
                lines.append(f'Packet {i}: skipped — Source has no main tooltip component or % = 0')
                continue
            extra=(comps[src][0]+comps[src][1])/2*pct/100
            if src.lower()==dst.lower():
                old=totals.get(dst,[0,0]); totals[dst]=[old[0]+extra,old[1]+extra]
                lines.append(f'Packet {i}: {src} → {dst}, {pct:.2f}% average → INCLUDED: +{extra:.2f}')
            else:
                separate_min+=extra; separate_max+=extra
                lines.append(f'Packet {i}: {src} → {dst}, {pct:.2f}% average → SEPARATE: {extra:.2f}–{extra:.2f}')
        ...
        lines += ['', 'Main tooltip by type:']
        for n,v in totals.items(): lines.append(f'  {n}: {fmt(v[0],v[1])}')
        lines += [f'MAIN TOOLTIP: {fmt(main_min,main_max)}',f'SEPARATE EXTRA PACKETS: {fmt(separate_min,separate_max)}',f'TOTAL HIT: {fmt(total_min,total_max)}','', '4. AFTER TOOLTIP']
        ...
        ef=1+cc/100*(cm-1)
        lines += ['', 'RULES:','• For each Increase, all sources are added; for each Amplification, sources are multiplied. The + button adds a new independent source; − zeros it.','• Damage % uses the same principle: additional Increase sources are added, additional Amplification sources are multiplied and then multiply the standard Amp.','• Weapon Range is completely excluded.','• Additional packet when Source = Target is merged with the main tooltip of this type; with different elements it remains separate.','• 17% packet currently uses the tested model: a percentage of source average damage, without reapplying target Inc/Amp.']
